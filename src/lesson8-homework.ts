...
function exercise42() {
  ...
  type TPhoneNumbersArray = TProduct["warehouse"]["phoneNumbers"];
  type TPhoneNumbers<T> = T extends (infer U)[] ? U : T;

  function getProductWarehousePhoneNumber(
    product: TProduct
  ): TPhoneNumbers<TPhoneNumbersArray> {
...
function exerciseExtra1() {
  ...
  function areAnagrams(s1: string, s2: string): boolean {
    ...
    const firstWord: TDictionary = {};
    const secondWord: TDictionary = {};
    for (let char of s1) {
      if (char in firstWord) {
        firstWord[char]++;
      } else firstWord[char] = 1;
    }
    for (let char of s2) {
      if (char in secondWord) {
        secondWord[char]++;
      } else secondWord[char] = 1;
    }
